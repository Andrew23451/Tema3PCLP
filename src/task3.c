...
StudentBlocks split_students_into_blocks(const student *studenti, size_t num_students) {
    ...
    size_t base_block_size = total_size / 4;
    size_t remainder = total_size % 4;
    size_t padded_block_size = base_block_size + (remainder > 0 ? 1 : 0);
...
void cripteaza_studenti(secretariat *secretariat, void *key, size_t key_len,
    ...
    for (int i = 0; i < secretariat->nr_studenti; i++) {
        StudentBlocks sb = split_students_into_blocks(secretariat->studenti, 1);
